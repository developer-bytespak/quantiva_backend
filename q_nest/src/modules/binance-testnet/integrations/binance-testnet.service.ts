...
export class BinanceTestnetService {
  ...
  /**
   * Creates a listen key for WebSocket user data stream
   * Note: userDataStream endpoints only need API key, not signature
   */
  async createListenKey(apiKey: string, apiSecret: string): Promise<string> {
    try {
      this.logger.log('Creating WebSocket listen key...');
      const response = await this.apiClient.post('/v3/userDataStream', {}, {
        headers: {
          'X-MBX-APIKEY': apiKey,
        },
      });
      this.logger.log(`Listen key created successfully: ${response.data.listenKey}`);
      return response.data.listenKey;
    } catch (error: any) {
      this.logger.error(`Failed to create listen key: ${error.message}`);
      if (error.response?.data) {
      ...
      // Re-throw with the actual error message from Binance if available
      const errorMsg = error.response?.data?.msg || error.message || 'Failed to create WebSocket listen key';
      throw new TestnetApiException(errorMsg);
    }
  }

  /**
   * Keeps the listen key alive (must be called every 30 minutes)
   * Note: userDataStream endpoints only need API key, not signature
   */
  async keepAliveListenKey(apiKey: string, apiSecret: string): Promise<void> {
    try {
      await this.apiClient.put('/v3/userDataStream', {}, {
        headers: {
          'X-MBX-APIKEY': apiKey,
        },
      });
    } catch (error: any) {
      this.logger.error(`Failed to keep alive listen key: ${error.message}`);
      if (error.response?.data) {
        this.logger.error(`API Response: ${JSON.stringify(error.response.data)}`);
      }
      throw new TestnetApiException('Failed to keep alive WebSocket listen key');
    }
  }
   ...
   * Note: userDataStream endpoints only need API key, not signature
   */
  async closeListenKey(apiKey: string, apiSecret: string): Promise<void> {
    try {
      await this.apiClient.delete('/v3/userDataStream', {
        headers: {
          'X-MBX-APIKEY': apiKey,
        },
      });
    } catch (error: any) {
      this.logger.error(`Failed to close listen key: ${error.message}`);
      if (error.response?.data) {
        this.logger.error(`API Response: ${JSON.stringify(error.response.data)}`);
      }
      throw new TestnetApiException('Failed to close WebSocket listen key');
    }
  }
}
