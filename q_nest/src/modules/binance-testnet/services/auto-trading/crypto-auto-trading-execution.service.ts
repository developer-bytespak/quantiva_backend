...
export class CryptoAutoTradingExecutionService {
  ...
  /**
   * Paper-trading "signal" rows used to be written into strategy_signals as a
   * side-effect of each paper auto-trade. Those signals were generated by a
   * Math.random() coin flip (see `generateSignal()` above), not by the real
   * fusion engine — so persisting them polluted the live signal table that
   * the top-trades page reads from. We disabled the write but keep the
   * function so paper trading's call site remains untouched.
   *
   * To re-enable safely: route writes to a separate `paper_strategy_signals`
   * table so they never mix with live engine signals.
   */
  private async storeSignal(
    _strategy: Strategy,
    _crypto: CryptoAsset,
    _signal: { action: 'BUY' | 'SELL'; confidence: number; scores: any },
    _amount: number,
    _price: number,
    ...
    /* ORIGINAL IMPLEMENTATION (kept for reference, never executed):
    try {
      await this.prisma.strategy_signals.create({
        data: {
          strategy_id: _strategy.strategy_id,
          asset_id: _crypto.asset_id,
          timestamp: new Date(),
          action: _signal.action,
          confidence: _signal.confidence,
          final_score: _signal.confidence * 100,
          sentiment_score: _signal.scores.sentiment_score,
          trend_score: _signal.scores.trend_score,
          fundamental_score: _signal.scores.fundamental_score,
          liquidity_score: _signal.scores.liquidity_score,
          volatility_score: _signal.scores.volatility_score,
          macro_score: _signal.scores.macro_score,
          engine_metadata: {
            crypto_auto_trade: true,
            trade_amount: _amount,
            trade_price: _price,
            stop_loss_price: _stopLossPrice,
            take_profit_price: _takeProfitPrice,
            testnet_order_id: _orderId,
