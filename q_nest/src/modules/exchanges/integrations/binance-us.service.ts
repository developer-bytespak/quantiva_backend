...
export class BinanceUSService {
  ...
  async convertDustToUsdt(
    ...
    try {
      ...
      // Step 2: convert ONLY the asset being closed — never bundle.
      //
      // Binance.US's 6h cooldown makes batching tempting, but the dust-assets
      // endpoint lists EVERY sub-$20 holding, so bundling means one "Sell XRP"
      // click liquidates all the user's other dust into USDT too. A
      // close-position sweep must touch only the position being closed, so we
      // pass a single-asset list and accept that clearing multiple dust
      // holdings takes one close (and one cooldown window) each.
      const result = await this.postDustConvert(apiKey, apiSecret, [requested], target);
      const transferred = parseFloat(result?.totalTransferred ?? '0') || 0;
      this.logger.log(
        `Dust-convert success: [${requested}] → ${transferred} ${target} (after fees)`,
      );
