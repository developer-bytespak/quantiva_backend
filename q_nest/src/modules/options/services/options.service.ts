...
import { estimateRiskReward } from './alpaca/risk-reward';
import {
  computeEv,
  computePop,
...
export class OptionsService {
  ...
  /**
   * Preview-shaped wrapper around the shared `estimateRiskReward` util
   * (services/alpaca/risk-reward.ts) — parses strikes off the legs' OCC
   * symbols, then delegates. Returns null for malformed symbols or unknown
   * strategy names so callers can fall back to whatever the signal had.
   */
  private estimateRiskRewardFromPreview(
    ...
    return estimateRiskReward(strategy, strikes, netPerUnit);
  }
